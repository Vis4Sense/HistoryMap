...
// }
//     if (title updated) send the new title to knowledgeMap.js through an event;
//     if (favIconUrl updated) send the new favIconUrl to knowledgeMap.js through an event;
// }

// function addNode(tab) {
//     create a new node with  the information from 'tab';
//     send the new 'node' to knowledgeMap.js through an event;
// }


knowledgeMap.controller.browser = function () {
	// const module = {};
	...
	var nodes = knowledgeMap.model.nodes;
	// var redraw = knowledgeMap.view.redraw(); // why this doesn't work?
	...
	function createActionObject(tabId, url, text, type, favIconUrl, path, classId, from, pic, hidden) {
		...
		knowledgeMap.model.nodes.addNode(action);
        nodeId++;
        return action;
	}
	...
	chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
		if (request.type === 'highlight') {
			...
			knowledgeMap.model.nodes.hideNode(request.tabUrl, request.classId);
			knowledgeMap.view.redraw();
		}
	});

	function onImageSaved(id, imageUrl) {
		var nodes = knowledgeMap.model.nodes.getArray();
