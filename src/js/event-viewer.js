$(function() {
    ...
    function extractSegments(browserData) {
        ...
        // So, should ignore it.
        // TODO Need to investigate the block below
        if (segments[0].view === 'col') {
            segments.shift();
            segments.shift(); // one participant started very early and did nothing for a while
        }
    ...
    function computeStats(segments, actions, columnNames, name) {
        ...
        segments.reduce((p, c) => {
            if (p.view !== c.view) {
                if (p.view === 'browser' && c.view === 'col' || p.view === 'col' && c.view === 'browser') {
                    browserCol++;
                }
                if (p.view === 'browser' && c.view === 'cur' || p.view === 'cur' && c.view === 'browser') {
                    browserCur++;
                }
                if (p.view === 'cur' && c.view === 'col' || p.view === 'col' && c.view === 'cur') {
                    colCur++;
                }
            }
